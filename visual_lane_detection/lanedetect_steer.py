...
def lane_detection(image, location='indoor'):
    if location == 'outdoor':
...
def lane_finding_pipeline_indoor(image):
    ...
    # Gaussian Smoothing
    smoothed_img = gaussian_blur(img = gray_img, kernel_size = 0)
...
def lane_finding_pipeline_outdoor(image):
    ...
    bc_img = brightness_contrast(Input_img = gray_img, contrast = 0, brightness = 0)
