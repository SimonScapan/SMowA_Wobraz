...
# 
# change wether you want indoor or outdoor data in line 22
#
...
for image_path in list(os.listdir(f'./visual_lane_detection/lane_detection_data/{location}')):
    # start timer for speed calculation of lane detection
    start = timer()

    # call lane detection for all pictures in folder
    image = mpimg.imread(f'./visual_lane_detection/lane_detection_data/{location}/{image_path}')
    try:
